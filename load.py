...
st.header('Datasets Obtained in the year 2023')
...
try:
  ...
  # add a filter          
    ...
    job_selection =  st.multiselect( "Job Title", job_titles, [job_titles[0], job_titles[2]])
                               
    selected_job_title = data[data['job_title'].isin(job_selection)]
    # table
    """## Top 5 rows """
    st.dataframe(selected_job_title.head()) 
    # pie chart
    # st.write(data.company.location.unique())
    # st.write(data.employment_type.unique())
    location = data.company_location.value_counts().head()
    explode = (0,0.1,0.2,0.1,0.2)
    fig1, ax1 = plt.subplots(figsize=(5,5))
    ax1.pie(location, labels=location.index, autopct='%1.1f%%')
    ax1.axis("equal")
